...
def replace_middle(input_list):
    ...
    """
    f_list = input_list[0:2]
    e_list = input_list[-2:]

    return (f_list + 42 + 37 + e_list)
...
def double_with_list_comprehension(input_list):
    ...
    new_list =[num*2 for num in input_list] 
...
def multiplication_table(n):
    ...
    """
    multtable = []
    for row in range(1, n + 1): #[1, 2, 3, 4, 5, ... , n]
        row_list = []
        for col in range(1, n + 1): #[1, 2, 3, 4, 5, ... , n]
            row_list.append(col * row)
        multtable.append(row_list)   
